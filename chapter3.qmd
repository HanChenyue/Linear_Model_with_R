---
title: "Linear Model with R: Chapter 3"
author: Han Chenyue
editor: source
format: html
---
...
90% CI for age does not include 0 but 95% CI for age does, therefore the p-value lies between 0.05 and 0.10.
...
-   In this case, the models rely on different functional forms of the predictor \[$log(x)$ vs $x$\], thus the column space of one model is not a linear subspace of the other.
...
-   None of the individual t-tests are statistically significant, yet the overall *F*-test is.
...
## Proof: Equivalence of *F*-test and t-test (for $a=2$)
...
### 2. The Denominator: Mean Square Error (MSE)
...
### 3. The Numerator: Mean Square Treatment (MST)
...
#### 3.1 Analyzing the deviation from the global mean
