---
title: "Linear Model with R: Chapter 2"
author: Han Chenyue
editor: source
format: html
---
...
cat(
  ...
  signif(cor(residuals(lm_q1a), predict(object = lm_q1a)), 3)
)
```
...
```{r}
all.equal(
  cor(predict(object = lm_q2a), uswages$wage)^2,
  summary(lm_q2a)$r.squared
)
```
...
```{r}
cor(predict(object = lm_q2e), uswages$wage)^2
```
...
RSS of Initial Model (More parameters, more flexibility) \< RSS of Intercept Removed Model (One less parameter)
...
-   All four regional indicators sum to one because every individual belongs to exactly one region; including all four indicators in the same model therefore leads to the identifiability problem.
