---
title: "Linear Model with R: Chapter 1"
author: Han Chenyue
editor: source
format: html
---
...
The x-axis is difficult to interpret because the tick marks align with the boundaries of the blocks, rather than being centred on them.

```{r}
plot(table(teengamb$verbal), xlab = "Verbal Score", ylab = "Frequency")
```

Cleaner, but the thin lines are a non-standard way of visualising a histogram.
...
A line plot is preferable for time-ordered data.
...
-   Also, both variables start low in the distant past as indicated by the darker coloured points and progress to higher values over time as indicated by the lighter coloured points.
